import { Head, Link, router } from '@inertiajs/react';
import AdminLayout from '@/layouts/admin/admin-layout';
import { Plus, Edit, Trash2 } from 'lucide-react';
import type { News } from '@/types';
import { Card, CardContent } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';

interface NewsIndexProps {
    news: News[];
    categories: string[];
}

export default function NewsIndex({ news, categories }: NewsIndexProps) {
    const handleDelete = (id: number) => {
        if (confirm('Are you sure you want to delete this news article?')) {
            router.delete(`/admin/news/${id}`);
        }
    };

    return (
        <AdminLayout>
            <Head title="Manage News" />

            <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between mb-6">
                <div>
                    <h1 className="text-2xl font-bold tracking-tight">News & Media</h1>
                    <p className="text-sm text-muted-foreground">Manage news articles and announcements.</p>
                </div>
                <Button asChild className="mt-3 sm:mt-0 w-fit">
                    <Link href="/admin/news/create">
                        <Plus className="h-4 w-4" />
                        Add Article
                    </Link>
                </Button>
            </div>

            <Card className="py-0 gap-0 overflow-hidden shadow-sm">
                <CardContent className="p-0">
                    <div className="overflow-x-auto">
                        <table className="w-full caption-bottom text-sm">
                            <thead className="[&_tr]:border-b bg-muted/50">
                                <tr className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                                    <th className="h-10 px-4 text-left align-middle font-medium text-muted-foreground">Title</th>
                                    <th className="h-10 px-4 text-left align-middle font-medium text-muted-foreground">Category</th>
                                    <th className="h-10 px-4 text-left align-middle font-medium text-muted-foreground">Published</th>
                                    <th className="h-10 px-4 text-left align-middle font-medium text-muted-foreground">Status</th>
                                    <th className="h-10 px-4 text-right align-middle font-medium text-muted-foreground">Actions</th>
                                </tr>
                            </thead>
                            <tbody className="[&_tr:last-child]:border-0">
                                {news.length > 0 ? (
                                    news.map((article) => (
                                        <tr key={article.id} className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                                            <td className="p-4 align-middle font-medium max-w-[320px] truncate">{article.title}</td>
                                            <td className="p-4 align-middle text-muted-foreground capitalize">
                                                {article.category ? (
                                                    <Badge variant="outline" className="font-normal capitalize">
                                                        {article.category}
                                                    </Badge>
                                                ) : (
                                                    <span className="text-xs text-muted-foreground">—</span>
                                                )}
                                            </td>
                                            <td className="p-4 align-middle text-muted-foreground">
                                                {article.published_at ? new Date(article.published_at).toLocaleDateString() : '-'}
                                            </td>
                                            <td className="p-4 align-middle">
                                                <Badge
                                                    variant={article.is_published ? 'secondary' : 'outline'}
                                                    className={
                                                        article.is_published
                                                            ? 'bg-green-500/15 text-green-700 border-green-200 dark:bg-green-500/10 dark:text-green-400 dark:border-green-900/50'
                                                            : 'bg-amber-500/15 text-amber-700 border-amber-200 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-900/50'
                                                    }
                                                >
                                                    {article.is_published ? 'Published' : 'Draft'}
                                                </Badge>
                                            </td>
                                            <td className="p-4 align-middle text-right">
                                                <div className="flex justify-end gap-1">
                                                    <Button variant="ghost" size="sm" asChild>
                                                        <Link href={`/admin/news/${article.id}/edit`}>
                                                            <Edit className="h-4 w-4" />
                                                            Edit
                                                        </Link>
                                                    </Button>
                                                    <Button
                                                        variant="ghost"
                                                        size="sm"
                                                        onClick={() => handleDelete(article.id)}
                                                        className="text-destructive hover:text-destructive hover:bg-destructive/10"
                                                    >
                                                        <Trash2 className="h-4 w-4" />
                                                        Delete
                                                    </Button>
                                                </div>
                                            </td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
                                        <td colSpan={5} className="p-8 text-center text-sm text-muted-foreground">
                                            No articles yet. Create your first news post.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
                    </div>
                </CardContent>
            </Card>
        </AdminLayout>
    );
}
